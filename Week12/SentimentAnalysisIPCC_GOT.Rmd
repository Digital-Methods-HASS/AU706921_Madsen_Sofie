...
```{r stopwords}
?stop_words
stop_words # Kan se stop words listen
...
ipcc_no_numeric <- ipcc_stop %>% 
  filter(is.na(as.numeric(word))) # as.numeric ("1", "abc") eller ("1", "NA") filtrer ord væk
```
...
length(unique(ipcc_no_numeric$word)) #unique -> Viser unikke ord
...
# Save it 
# Ser hvilke mest forekomne ord, der passer til, hvilke følelser
...
The most common meaningful words is king, lord and sir, and we expect that the emotions will be anger, fear, sad, surprise which it was.
...
### Get the GOT report:
```{r get-document Game of Thrones}
...
Now check the counts again: 
```{r count-words2 Game of Thrones}
got_swc <- got_stop %>% 
  count(word) %>% 
  arrange(-n)
got_swc
...
What if we want to get rid of all the numbers (non-text) in `ipcc_stop`?
```{r skip-numbers Game of Thrones}
# This code will filter out numbers by asking:
# If you convert to as.numeric, is it NA (meaning those words)?
# If it IS NA (is.na), then keep it (so all words are kept)
# Anything that is converted to a number is removed
...
```{r wordcloud-prep Game of Thrones}
# There are almost 2000 unique words 
length(unique(ipcc_no_numeric$word)) #unique -> Viser unikke ord
...
That's underwhelming. Let's customize it a bit:
```{r wordcloud-pro Game of Thrones}
...
First, bind words in `ipcc_stop` to `afinn` lexicon:
```{r bind-afinn Game of Thrones}
got_afinn <- got_stop %>% 
  inner_join(get_sentiments("afinn"))
```

Let's find some counts (by sentiment ranking):
```{r count-afinn Game of Thrones}
...
```{r afinn-2-more Game of Thrones}
# Check the unique 2-score words:
...
Or we can summarize sentiment for the report: 
```{r summarize-afinn Game of Thrones}
...
```{r check-exclusions Game of Thrones}
...
Wait, so "confidence" is showing up in NRC lexicon as "fear"? Let's check:
```{r nrc-confidence Game of Thrones}
conf <- get_sentiments(lexicon = "nrc") %>% 
  filter(word == "confidence")
